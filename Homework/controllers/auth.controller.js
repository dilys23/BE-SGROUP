...
const { generateOTP, sendOTPEMail } = require('../utils/sendResetPassEmail.js');
...
const validateEmail = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Email valid in DB', email: result.email });
    } catch (error) {
        res.status(500).send('An error occurred');
    }
};
...
{
    const {email, otp, newpassword, cfmpassword} = req.body;
